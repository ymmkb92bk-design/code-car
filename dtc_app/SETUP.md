...
- ~~Self-service data deletion~~ — "حذف بياناتي" in the Settings screen, backed by the `delete_my_data` RPC (`supabase/migrations/005_delete_my_data.sql`). Wipes the device's `search_logs`, `daily_usage`, and `users` rows immediately on confirmation — this is the real PDPL "right to deletion" mechanism, not just an email promise.
