...
- ~~Self-service data deletion~~ — "حذف بياناتي" in the Settings screen, backed by the `delete_my_data` RPC (`supabase/migrations/006_keep_quota_on_delete.sql`). Wipes the device's `search_logs` and `users` rows immediately on confirmation — this is the real PDPL "right to deletion" mechanism, not just an email promise. `daily_usage` (the daily quota counter) is deliberately kept — deleting it would let anyone reset their free-search limit just by tapping delete.
